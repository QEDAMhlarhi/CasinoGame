...
def build_card(player, table):
    ...
    try:
        ...
        # Find possible builds using cards from the table
...
#the Winner
player_points = sum(int(card['value']) for card in players[0]['hand'])
computer_points = sum(int(card['value']) for card in players[1]['hand'])
